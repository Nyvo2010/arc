from __future__ import annotations
import argparse, csv, json, time, os
from pathlib import Path
from arc.models.registry import MODEL_VARIANTS
from arc.inference import InferenceEngine
import torch

def set_seed(seed=0):
    torch.manual_seed(seed)
    torch.cuda.manual_seed_all(seed)

def main():
    p = argparse.ArgumentParser()
    p.add_argument('--source', required=True)
    p.add_argument('--device', default='cpu')
    p.add_argument('--outdir', required=True)
    args = p.parse_args()
    out = Path(args.outdir)
    out.mkdir(parents=True, exist_ok=True)
    set_seed(0)
    
    # simple synthetic prompts
    prompts = [torch.randint(2, 128, (1,128)) for _ in range(32)]
    
    rows = []
    for v in MODEL_VARIANTS:
        print(f"[benchmark_all] {v}")
        engine = InferenceEngine(source=args.source, variant=v, device_map='auto' if args.device!='cpu' else None)
        total_t=0
        flops=0
        for ids in prompts:
            ids = ids.to(args.device) if args.device!='cpu' else ids
            m = engine.measure(ids)
            total_t += m['elapsed_s']
            flops += m['compute_used']
        rows.append({
            'variant': v,
            'scale': MODEL_VARIANTS[v]['scale'],
            'adaptive': MODEL_VARIANTS[v]['adaptive'],
            'avg_time_s': total_t/len(prompts),
            'avg_flops': flops/len(prompts)
        })
    csv_path = out/'synthetic_summary.csv'
    with csv_path.open('w',newline='') as f:
        w = csv.DictWriter(f, fieldnames=['variant','scale','adaptive','avg_time_s','avg_flops'])
        w.writeheader()
        w.writerows(rows)
    print(f"[benchmark_all] Wrote {csv_path}")

if __name__=='__main__':
    main()
